...
void ExecuteCommand(DirectoryTree* dirTree, char* cmd) {
    ...
    } else if (strcasecmp(cmd, "chown") == 0) {
      cmd = strtok(NULL,""); 
        ft_chown(dirTree,cmd);
...
void Start()
{
    ...
    printf("%d %02d:%02d:%02d %d\n", userList->current->day, userList->current->hour, userList->current->minute, userList->current->sec, userList->current->year);
}
